...
export default function App() {
  ...
  return <main className="app">
    ...
    <section className={`table ${logOpen ? '' : 'table-log-hidden'}`}>
      ...
      {G.pending?.kind === 'persona_11_offer' && String(G.pending.playerId) === '0' && <aside className="controls"><button onClick={() => client.moves.persona11Use()}>{language === 'en' ? 'Use Solovyov' : 'Использовать Соловьёва'}</button><button onClick={() => client.moves.persona11Skip()}>{language === 'en' ? 'Skip Solovyov' : 'Пропустить Соловьёва'}</button></aside>}
    </section>
