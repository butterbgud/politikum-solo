...
export default function App() {
  ...
  return <main className="app">
    ...
    <section className="table">
      ...
      <aside className="controls"><button disabled={!active || !!G.pending || !!G.response || G.hasPlayed || Number(G.drawsThisTurn || 0) >= 2} onClick={() => client.moves.drawCard()}>{Number(G.drawsThisTurn || 0) === 1 ? ui.secondDraw : ui.draw}</button><button disabled={!active || !!G.pending || !!G.response || !G.hasDrawn || !G.hasPlayed} onClick={() => client.moves.endTurn()}>{ui.end}</button>{G.pending?.kind === 'persona_11_offer' && String(G.pending.playerId) === '0' && <><button onClick={() => client.moves.persona11Use()}>{language === 'en' ? 'Use Solovyov' : 'Использовать Соловьёва'}</button><button onClick={() => client.moves.persona11Skip()}>{language === 'en' ? 'Skip Solovyov' : 'Пропустить Соловьёва'}</button></>}{G.pending && String(G.pending.playerId ?? G.pending.attackerId) === '0' && G.pending.kind !== 'persona_11_offer' && <button className="resolve" onClick={resolveFirstChoice}>{ui.auto}</button>}<small>{G.response ? `Окно ответа: ${responseSeconds}с` : ui.playAfterDraw}</small></aside>
    </section>
