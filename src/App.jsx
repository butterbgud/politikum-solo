...
const UI = {
  ru: { rivals: 'Соперники', start: 'Начать локальную игру', intro: 'Всё состояние живёт в браузере. Никаких аккаунтов, API или сервера.', salon: 'Политический салон', yourTurn: 'Ваш ход', thinking: 'думает', deck: 'карт в колоде', newGame: 'Новая игра', log: 'Хроника', you: 'Вы', draw: 'Взять карту', secondDraw: 'Взять вторую · конец хода', end: 'Конец хода', auto: 'Разрешить выбор', choose: 'Выбрать', close: 'Нажмите вне карты, чтобы закрыть', ended: 'Партия окончена', won: 'Вы победили', wins: 'побеждает', again: 'Ещё одну', reportBug: 'Сообщить об ошибке', reportTitle: 'Сообщить об ошибке', reportHint: 'К отчёту будет приложена последняя история партии.', reportPlaceholder: 'Что произошло? (необязательно)', challenge: 'Решите пример, чтобы отправить отчёт:', challengeWrong: 'Неверный ответ. Отчёт не отправлен.', send: 'Отправить', cancel: 'Отмена', sent: 'Отчёт отправлен. Спасибо!', failed: 'Не удалось отправить отчёт.' },
...
function Card({ card, language, onClick, onPreview, dim = false, selected = false, showVictory = true, directClick = false }) {
  if (!card) return null;
  ...
  const inspectOrPlay = () => {
    if (!directClick && window.matchMedia('(hover: none), (pointer: coarse)').matches) onPreview?.(card, onClick);
    else onClick?.();
  };
...
function pendingText(pending, language) {
  const copy = language === 'en' ? {
    ...
    persona_20_pick_from_discard: 'Быков: выберите карту действия из сброса.',
    ...
    persona_9_choose_opponent: 'Пономарёв: выберите соперника, в чью коалицию его сыграть.',
...
export default function App() {
  ...
  const [bugChallenge, setBugChallenge] = useState(() => makeBugChallenge());
  ...
  useEffect(() => {
    if (!client || !G || !active || G.gameOver || G.response || G.pending || G.hasDrawn) return undefined;
    const timer = setTimeout(() => client.moves.drawCard(), 120);
    return () => clearTimeout(timer);
  }, [client, G, active]);
  ...
  useEffect(() => {
    if (G?.pending?.kind !== 'persona_3_choice') setSvtvPicking(false);
  }, [G?.pending?.kind, G?.pending?.sourceCardId]);
  ...
  const play = (card) => {
    ...
    if (card.type === 'persona') {
      client.moves.playPersona(card.id);
  ...
  const resolveFirstChoice = () => {
    ...
    if (pending.kind === 'persona_7_swap_two_in_coalition') { const host = G.players.find((player) => player.coalition.length > 1); return host ? call('persona7SwapTwoInCoalition', host.id, host.coalition[0].id, host.coalition[1].id) : call('persona7Skip'); }
    ...
    if (pending.kind === 'persona_20_pick_from_discard') return call('persona20PickFromDiscard', G.discard.find((card) => card.type === 'action')?.id);
  ...
  const openBug = () => {
    setBugStatus('');
    setBugAnswer('');
    setBugChallenge(makeBugChallenge());
    setBugOpen(true);
  };
  const submitBug = async () => {
    if (Number(bugAnswer) !== bugChallenge.answer) {
      setBugStatus('wrong-answer');
      return;
    }
  ...
  const responseCards = new Set(canAnswerResponse ? (G.response.kind === 'cancel_action'
    ? ['action_6', ...((String(G.pending?.targetId) === '0' || G.pending?.kind === 'action_17_choose_opponent_persona') ? ['action_14'] : [])]
    : G.response.kind === 'cancel_persona' && baseId(G.response.personaCard?.id || '') !== 'persona_33' ? ['action_8']
      : G.response.kind === 'cancel_persona_ability' ? ['action_14'] : []) : []);
  ...
  const event12bDiscardPending = G.pending?.kind === 'event_12b_discard_from_hand'
    && (G.pending.targetIds || []).some((id) => String(id) === '0');
  ...
  const arnoTarget = G.pending?.kind === 'persona_17_pick_persona_from_hand'
    ...
    : null;
  const persona11Blocked = !!me?.coalition?.some((card) => baseId(card.id) === 'persona_11' && card.blockedAbilities);
  ...
  return <main className="app">
    ...
    {event12bDiscardPending && <div className="discard-picker-modal"><section className="discard-picker"><b>{language === 'en' ? 'Twitter Squabble — choose a card to discard' : 'Секс скандал — выберите карту для сброса'}</b><small>{language === 'en' ? 'Choose one card from your hand.' : 'Выберите одну карту из руки.'}</small><div className="fan">{(me?.hand || []).map((card) => <Card card={card} language={language} key={card.id} onClick={() => client.moves.discardFromHandForEvent12b(card.id)} onPreview={(picked, action) => setPreview({ card: picked, action })} />)}</div></section></div>}
    ...
    <section className={`table ${logOpen ? '' : 'table-log-hidden'}`}>
      ...
      {G.pending?.kind === 'persona_11_offer' && String(G.pending.playerId) === '0' && !persona11Blocked && <div className="discard-picker-modal persona11-modal"><section className="discard-picker persona11-picker persona11-offer"><b>{language === 'en' ? 'Use Solovyov’s ability?' : 'Использовать способность Соловьёва?'}</b><small>{language === 'en' ? 'Skip drawing a card and discard an opponent persona, but discard Solovyov too.' : 'Пропустите добор и сбросьте персону соперника, но Соловьёв тоже будет сброшен.'}</small><div className="persona11-actions"><button onClick={() => client.moves.persona11Use()}>{language === 'en' ? 'Use ability' : 'Использовать'}</button><button onClick={() => client.moves.persona11Skip()}>{language === 'en' ? 'Skip' : 'Пропустить'}</button></div></section></div>}
    {persona11Targeting && <div className="discard-picker-modal persona11-modal"><section className="discard-picker persona11-picker"><b>{language === 'en' ? 'Solovyov — choose an opponent persona to discard' : 'Соловьёв — выберите персону соперника для сброса'}</b><small>{language === 'en' ? 'Click a visible opponent persona. Solovyov will discard himself too.' : 'Нажмите на видимую персону соперника. Соловьёв также будет сброшен.'}</small><div className="persona11-targets">{G.players.filter((player) => player.active && player.id !== '0').flatMap((player) => (player.coalition || []).filter((card) => card.type === 'persona' && baseId(card.id) !== 'persona_31' && !card.shielded).map((card) => ({ player, card }))).map(({ player, card }) => <Card card={card} language={language} key={`${player.id}-${card.id}`} directClick onClick={() => client.moves.persona11DiscardOpponentPersona(player.id, card.id)} />)}</div></section></div>}
    </section>
    {G.pending?.kind === 'persona_3_choice' && String(G.pending.playerId) === '0' && <div className="discard-picker-modal"><section className="discard-picker svtv-choice"><b>SVTV</b>{svtvPicking ? <><small>{language === 'en' ? 'Choose one left-wing resident to discard.' : 'Выберите одного левого персонажа для сброса.'}</small><div className="fan">{G.players.flatMap((player) => (player.coalition || []).filter(isSvtvTarget).map((card) => ({ player, card }))).map(({ player, card }) => <Card card={card} language={language} key={card.id} onClick={() => client.moves.persona3ChooseOption('a', player.id, card.id)} onPreview={(picked, action) => setPreview({ card: picked, action })} />)}</div><button onClick={() => setSvtvPicking(false)}>{language === 'en' ? 'Back' : 'Назад'}</button></> : <div><button onClick={() => setSvtvPicking(true)}>{language === 'en' ? 'Discard 1 left-wing resident' : 'Сбросить 1 левую персону'}</button><button onClick={() => client.moves.persona3ChooseOption('b')}>{language === 'en' ? 'Give all left-wing residents −1' : 'Дать всем левым −1'}</button><button onClick={() => client.moves.persona3Skip()}>{language === 'en' ? 'Skip SVTV ability' : 'Пропустить способность СВТВ'}</button></div>}</section></div>}
    ...
    {G.pending?.kind === 'persona_20_pick_from_discard' && String(G.pending.playerId) === '0' && <div className="discard-picker-modal"><section className="discard-picker"><b>{language === 'en' ? 'Bykov — choose an action card' : 'Быков — выберите карту действия'}</b><div className="fan">{G.discard.filter((card) => card.type === 'action').map((card) => <Card card={card} language={language} key={card.id} onClick={() => client.moves.persona20PickFromDiscard(card.id)} onPreview={(picked, action) => setPreview({ card: picked, action })} />)}</div></section></div>}
    {G.pending?.kind === 'persona_23_choose_self_inflict_draw' && String(G.pending.playerId) === '0' && <div className="discard-picker-modal"><section className="discard-picker persona23-choice"><b>{language === 'en' ? 'Volkov — choose negative VP tokens' : 'Волков — выберите отрицательные жетоны VP'}</b><small>{language === 'en' ? 'Click an option to confirm immediately.' : 'Нажмите на вариант, чтобы сразу подтвердить выбор.'}</small><div>{[1, 2, 3].map((n) => <button key={n} disabled={n > 3 - Number(G.pending.taken || 0)} onClick={() => client.moves.persona23ChooseSelfInflict(n)}>{language === 'en' ? `−${n} VP · draw ${n}` : `−${n} VP · взять ${n}`}</button>)}</div></section></div>}
    {G.pending?.kind === 'persona_33_choose_faction' && String(G.pending.playerId) === '0' && <div className="discard-picker-modal"><section className="discard-picker sobchak-choice"><b>{language === 'en' ? 'Sobchak — choose a faction' : 'Собчак — выберите фракцию'}</b><small>{language === 'en' ? 'She gains +1 for every matching resident in your coalition, including herself.' : 'Она получит +1 за каждого совпадающего персонажа этой фракции в вашей коалиции, включая себя.'}</small><div>{[['faction:liberal', language === 'en' ? 'Liberal' : 'Либералы'], ['faction:rightwing', language === 'en' ? 'Right-wing' : 'Правые'], ['faction:leftwing', language === 'en' ? 'Left-wing' : 'Левые'], ['faction:fbk', 'FBK'], ['faction:red_nationalist', language === 'en' ? 'Red Nationalist' : 'Красные националисты'], ['faction:system', language === 'en' ? 'System' : 'Системные']].map(([tag, label]) => <button key={tag} onClick={() => client.moves.persona33ChooseFaction(tag)}>{label}</button>)}</div></section></div>}
    ...
    {bugOpen && <div className="bug-modal" onClick={() => bugStatus !== 'sending' && setBugOpen(false)}><form onSubmit={(event) => { event.preventDefault(); submitBug(); }} onClick={(event) => event.stopPropagation()}><h2>{ui.reportTitle}</h2><p>{ui.reportHint}</p><textarea autoFocus value={bugText} onChange={(event) => setBugText(event.target.value)} placeholder={ui.reportPlaceholder} maxLength="1200" /><label className="bug-challenge">{ui.challenge}<strong>{bugChallenge.left} × {bugChallenge.right} =</strong><input inputMode="numeric" pattern="[0-9]*" value={bugAnswer} onChange={(event) => { setBugAnswer(event.target.value); if (bugStatus === 'wrong-answer') setBugStatus(''); }} /></label>{bugStatus === 'sent' ? <strong className="bug-success">{ui.sent}</strong> : bugStatus === 'failed' ? <strong className="bug-failed">{ui.failed}</strong> : bugStatus === 'wrong-answer' ? <strong className="bug-failed">{ui.challengeWrong}</strong> : null}<div><button type="button" onClick={() => setBugOpen(false)} disabled={bugStatus === 'sending'}>{ui.cancel}</button><button className="start" type="submit" disabled={bugStatus === 'sending' || bugStatus === 'sent'}>{ui.send}</button></div></form></div>}
