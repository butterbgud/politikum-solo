...
export default function App() {
  ...
  return <main className="app">
    ...
    {G.pending?.kind === 'persona_11_offer' && String(G.pending.playerId) === '0' && <section className="persona11-choice"><b>{language === 'en' ? 'Solovyov — use his ability before drawing?' : 'Соловьёв — использовать способность до добора?'}</b><small>{language === 'en' ? 'Discard Solovyov and one unprotected opponent resident; skip your draw.' : 'Сбросьте Соловьёва и незащищённого персонажа соперника; добор будет пропущен.'}</small><div><button className="start" onClick={() => client.moves.persona11Use()}>{language === 'en' ? 'Use ability' : 'Использовать способность'}</button><button onClick={() => client.moves.persona11Skip()}>{language === 'en' ? 'Skip' : 'Пропустить'}</button></div></section>}
