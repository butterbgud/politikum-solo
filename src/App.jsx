...
export default function App() {
  ...
  return <main className="app">
    ...
    <section className="table">
      ...
      <aside className="controls">{G.pending?.kind === 'persona_11_offer' && String(G.pending.playerId) === '0' && <><button onClick={() => client.moves.persona11Use()}>{language === 'en' ? 'Use Solovyov' : 'Использовать Соловьёва'}</button><button onClick={() => client.moves.persona11Skip()}>{language === 'en' ? 'Skip Solovyov' : 'Пропустить Соловьёва'}</button></>}{G.pending && String(G.pending.playerId ?? G.pending.attackerId) === '0' && !['persona_11_offer', 'action_4_discard_cost', 'action_4_choose_target', 'persona_20_pick_from_discard'].includes(G.pending.kind) && <button className="resolve" onClick={resolveFirstChoice}>{ui.auto}</button>}<small>{G.response ? `Окно ответа: ${responseSeconds}с` : ui.playAfterDraw}</small></aside>
    </section>
