...
export default function App() {
  ...
  return <main className="app">
    ...
    {G.pending?.kind === 'persona_11_offer' && String(G.pending.playerId) === '0' && <div className="discard-picker-modal"><section className="discard-picker"><b>{language === 'en' ? 'Solovyov — use his ability before drawing?' : 'Соловьёв — использовать способность до добора?'}</b><small>{language === 'en' ? 'Discard Solovyov and one unprotected opponent resident; skip your draw.' : 'Сбросьте Соловьёва и незащищённого персонажа соперника; добор будет пропущен.'}</small><div><button className="start" onClick={() => client.moves.persona11Use()}>{language === 'en' ? 'Use ability' : 'Использовать способность'}</button><button onClick={() => client.moves.persona11Skip()}>{language === 'en' ? 'Skip' : 'Пропустить'}</button></div></section></div>}
    ...
    {winner && <div className={`ending ${milovVictory ? 'milov-ending' : ''}`}>{String(G.winnerId) === '0' && <div className="fireworks" aria-hidden="true">{Array.from({ length: 30 }, (_, index) => <i key={index} style={{ '--x': `${(index * 37) % 100}%`, '--delay': `${(index % 9) * -0.27}s`, '--dx': `${(index % 7 - 3) * 48}px`, '--dy': `${(index % 5 - 3) * 54}px`, '--hue': index * 31 }} />)}</div>}<div>{milovVictory ? <><p>{language === 'en' ? 'A momentous prediction!' : 'Судьбоносное предсказание!'}</p><h2>{winner.id === '0' ? (language === 'en' ? 'Milov has seen the future — you win!' : 'Милов увидел будущее — вы победили!') : (language === 'en' ? `${winner.name} saw the future!` : `${winner.name} увидел будущее!`)}</h2><strong>✦ MILOV ✦</strong></> : <><p>{ui.ended}</p><h2>{winner.id === '0' ? ui.won : `${winner.name} ${ui.wins}`}</h2><strong>{score(winner)} VP</strong></>}<ScoreChart history={G.history} players={G.players.filter((player) => player.active)} /><button onClick={start}>{ui.again}</button></div></div>}
  </main>;
}
