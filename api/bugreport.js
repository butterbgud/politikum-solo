...
export default async function handler(req, res) {
  ...
  const message = ['🐛 Politikum Solo bug report', `Version: ${limit(body.version, 80) || 'unknown'}`, `Language: ${body.language === 'en' ? 'English' : 'Russian'}`, `URL: ${limit(body.url, 500) || 'unknown'}`, `State: turn ${limit(body.game?.turn, 30) || '?'} · pending ${limit(body.game?.pending, 100) || 'none'} · response ${limit(body.game?.response, 100) || 'none'} · deck ${limit(body.game?.deck, 30) || '?'}`, `Browser: ${limit(body.userAgent, 400) || 'unknown'}`, '', `Note: ${limit(body.text, 1200) || '(none)'}`, '', 'Recent history:', ...(history.length ? history : ['(no game history)'])].join('\n').slice(0, 3900);
  try {
